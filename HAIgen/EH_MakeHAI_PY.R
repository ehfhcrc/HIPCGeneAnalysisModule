...
makeHAI <- function(sdy){
  ...
  # remove subjects to mimic original. However, still need 8 rows of info!!??!!
  if(sdy == "SDY80"){
    # determined by mapping all biosampleIDs in original file to participantIDs
    # and then determining those participants that were not in the original
    # but do get generated by an ImmuneSpace version. NOTE: biosampleID 223 did
    # not map to any participantID.
    subs_to_rm <- c("SUB114455.80","SUB114466.80","SUB114486.80","SUB114489.80",
                    "SUB114490.80","SUB114497.80","SUB114501.80", "SUB114509.80",
                    "SUB114510.80","SUB114484.80")
    titer_data <- subset(titer_data, !(titer_data$subject %in% subs_to_rm))
    # Missing IDS: 114456, 114457, and 114507 all are available but the titers are 
    # backwards going from d0 to dneg7.  114479 is available but titer d7 to d70.
    # 114485 and 114504 have only one titer.  114465 and 114496 have no data in IS.
  }
  ...
  # single member bins, therefore third bin was added for 6.
  SDY400_bins <- list(c(1,5,6),c(1,5,6),c(1,5,6))
