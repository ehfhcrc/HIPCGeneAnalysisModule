...
#----------------PARAMETERS-------------------------------------
# EH NOTE: Change file directories to local machine
SIG_dir <- "/home/ehenrich/R/HIPCGeneModuleAnalysis/"
HAI_dir <- file.path(SIG_dir,'HAI')
DATA_dir <- file.path(SIG_dir, 'GE')

# EH Note: Important to set 'force' to True for first run otherwise code does not pick up 
# lack of Rds files ... not sure why not.
...
# EH NOTE: Finds sdy files and sets up objects to hold info
if( !use_hai_combined ){
...
l_ply(dfile, function(f){
  ...
  if(sdy == "SDY67") {
    if(require(DESeq)) {
      ...
      ## dups <- duplicated(fdata$geneSymbol)
      ## sum(dups)
      countTable <- e
      ...
      ## check size factor
      ## sizeFactors(cds)
      ## ## First way, now replaced by VST below
      ## ## normalize the data using size factor and  then log2 transform
      ## ## Add 1 as a 'dummy' count so that log2 doesn't give -Inf
      ## norm.Data <- log2(counts(cds, normalized=TRUE)+1)
      ## e <- norm.Data
